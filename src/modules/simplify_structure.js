 ...
 */
function isObjectMember(node, propName) {
 ...
 */
function isObjectPrototypeMember(node) {
    if (!t.isMemberExpression(node)) return false;
    if (!t.isMemberExpression(node.object)) return false;
    if (!t.isIdentifier(node.object.object, { name: 'Object' })) return false;
    
    const protoKey = node.object.property;
    const isProto = t.isStringLiteral(protoKey, { value: 'prototype' }) || 
 ...
 */
function replaceBindingReferences(path, idName, replacer) {
 ...
 */
function createMemberAssignment(args) {
    ...
    if (t.isStringLiteral(prop) && isValidIdentifier(prop.value)) {
        assignment.left.computed = false;
        assignment.left.property = t.identifier(prop.value);
    }
    return assignment;
}
 ...
 */
function matchesAssignPolyfill(path) {
    let hasForIn = false;
    let hasHasOwn = false;
    
    path.traverse({
        ForInStatement() { hasForIn = true; },
        Identifier(p) { if (p.node.name === 'hasOwnProperty') hasHasOwn = true; },
        StringLiteral(p) { if (p.node.value === 'hasOwnProperty') hasHasOwn = true; },
        noScope: true
    });
 ...
 */
function hasAsyncIteratorUsage(path) {
    let found = false;
    path.traverse({
        MemberExpression(p) {
            if (t.isIdentifier(p.node.object, { name: 'Symbol' }) &&
 ...
 */
function restoreObjectShortcuts(ast) {
    ...
    traverse(ast, {
        VariableDeclarator(path) {
            ...
            if (isObjectMember(init, 'defineProperty')) {
                ...
                replacement = t.memberExpression(t.identifier('Object'), t.identifier('getOwnPropertySymbols'));
            } else {
        ...
        'ClassMethod|ObjectMethod|ClassProperty|ObjectProperty'(path) {
 ...
 */
function cleanSymbolPolyfills(ast) {
 ...
 */
function simplifyDefinePropertyHelpers(ast, path) {
    const { id, init } = path.node;
 ...
 * Pattern: (a, b, c) => a[...b...] = c
 */
function simplifyAssignmentHelpers(ast, path) {
    const { id, init } = path.node;
 ...
 */
function replaceObjectAssignPolyfills(ast, path) {
    const { id, init } = path.node;
    
    if (!t.isArrowFunctionExpression(init) || init.params.length !== 2) return false;
    if (!t.isBlockStatement(init.body)) return false;
    if (!matchesAssignPolyfill(path.get('init.body'))) return false;
 ...
 */
function renameAsyncIteratorHelpers(ast, path) {
    const { id, init } = path.node;
 ...
 */
function simplifyHelperFunctions(ast) {
    ...
    traverse(ast, {
        VariableDeclarator(path) {
            ...
            if (simplifyDefinePropertyHelpers(ast, path)) return;
            if (simplifyAssignmentHelpers(ast, path)) return;
            if (replaceObjectAssignPolyfills(ast, path)) return;
            renameAsyncIteratorHelpers(ast, path);
        }
    });
}
